...
class ROIAlign:
    ...
    def backward(self, dy):
        ...
        if self.req[0] != req.add:
            self.dX[0][:] = 0
        data, rois = self.X
        mobula.func.roi_align_backward(dy.size, dy, self.spatial_scale, data.shape[1], data.shape[2], data.shape[3], self.pooled_size[0], self.pooled_size[1], self.sampling_ratio, self.dX[0], rois)
