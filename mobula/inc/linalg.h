...
MOBULA_KERNEL linalg_gemm_tf_kernel(const int n, const T *a, const T *b,
        const int I, const int U, const int J, T *out) {
    for (int u = 0; u < U; ++u) {
        parfor(n, [&](int index) {
            int i = index / J;
            int j = index % J;
...
void linalg_gemm_ff(const DType *a, const DType *b,
        const int I, const int U, const int J, DType *out) {
#if not USING_CBLAS
    const int N = I;
...
void linalg_gemm_ft(const DType *a, const DType *b,
        const int I, const int U, const int J, DType *out) {
#if not USING_CBLAS
    const int N = I * J;
...
void linalg_gemm_tf(const DType *a, const DType *b,
        const int I, const int U, const int J, DType *out) {
#if not USING_CBLAS
    const int N = I * J;
...
void linalg_gemm_tt(const DType *a, const DType *b,
        const int I, const int U, const int J, DType *out) {
#if not USING_CBLAS
    const int N = J;
